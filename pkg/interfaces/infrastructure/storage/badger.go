...
type BadgerTransaction interface {
	...
	// 返回值为 TxSizeEstimator 接口，可用于监控事务大小
	// 如果未启用大小估算，可能返回 nil
	GetSizeEstimator() TxSizeEstimator
}

// TxSizeEstimator 事务大小估算器接口
//
// 用于估算 BadgerDB 事务的大小，避免超过10MB限制
type TxSizeEstimator interface {
	// AddWrite 记录写入操作
	AddWrite(keyLen, valueLen int)

	// AddDelete 记录删除操作
	AddDelete(keyLen int)

	// GetCurrentSize 获取当前事务大小估算值（字节）
	GetCurrentSize() uint64

	// IsNearLimit 检查是否接近限制（80%阈值）
	IsNearLimit() bool

	// Reset 重置估算器
	Reset()

	// GetUsagePercent 获取使用百分比（0-100）
	GetUsagePercent() float64

	// GetMaxSize 获取最大事务大小限制（字节）
	GetMaxSize() uint64

	// GetRemainingSize 获取剩余可用空间（字节）
