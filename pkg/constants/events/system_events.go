...
const (
	...
	// 🔧 Phase 3: Discovery间隔重置事件
	// EventTypeDiscoveryIntervalReset Discovery间隔重置事件
	// 发布者：network_notifiee（peer断连）、Kademlia（路由表退化）、AddrManager（地址过期）
	// 订阅者：discovery.Service（重置scheduler和DHT循环间隔）
	EventTypeDiscoveryIntervalReset EventType = "network.discovery.interval_reset"

	// 网络peer连接事件
	// EventTypeNetworkPeerConnected peer连接成功事件
	// 发布者：network_notifiee
	// 订阅者：Kademlia（更新健康分）、AddrManager（更新地址TTL）
	EventTypeNetworkPeerConnected EventType = "network.peer.connected"

	// EventTypeNetworkPeerDisconnected peer断连事件
	// 发布者：network_notifiee
	// 订阅者：Kademlia（更新健康分）、discovery.Service（可能触发重置）
