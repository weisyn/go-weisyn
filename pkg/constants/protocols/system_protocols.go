...
func GetProtocolCategory(protocol string) string {
	switch {
	case protocol == ProtocolNodeInfo || protocol == ProtocolHeartbeat || protocol == ProtocolPeerDiscovery:
		return "node_management"
	case protocol == ProtocolHealthCheck || protocol == ProtocolStatusSync:
		return "monitoring"
	case protocol == ProtocolBlockSync || protocol == ProtocolHeaderSync || protocol == ProtocolStateSync:
		return "blockchain_sync"
	case protocol == ProtocolConsensusCoordination || protocol == ProtocolNetworkQualityReport:
