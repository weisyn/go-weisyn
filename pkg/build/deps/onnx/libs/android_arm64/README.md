...
本目录用于存放 Android ARM64 平台的 ONNX Runtime 库文件和嵌入代码。
...
- `--android_abi arm64-v8a`: 指定 ARM64 架构
...
case "android_arm64":
    if len(libAndroidARM64) == 0 {
        return nil, fmt.Errorf("嵌入的库文件为空 (android_arm64)。请参考 libs/android_arm64/embedded.go")
    }
...
6. **测试验证**：添加支持后，务必在 Android ARM64 设备上进行完整测试
