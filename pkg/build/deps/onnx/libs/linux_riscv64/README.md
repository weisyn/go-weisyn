...
本目录用于存放 Linux RISCV64 平台的 ONNX Runtime 库文件和嵌入代码。

## 📋 平台信息

- **操作系统**: Linux
- **架构**: RISCV64 (RISC-V 64-bit)
...
```bash
# 安装 RISC-V 交叉编译工具链
# 参考：https://github.com/riscv/riscv-gnu-toolchain

# 配置交叉编译
./build.sh --config Release --build_shared_lib \
  --cmake_extra_defines CMAKE_C_COMPILER=riscv64-linux-gnu-gcc \
  --cmake_extra_defines CMAKE_CXX_COMPILER=riscv64-linux-gnu-g++
```
...
```bash
cp build/Linux/Release/libonnxruntime.so pkg/build/deps/onnx/libs/linux_riscv64/libonnxruntime.so
```
...
case "linux_riscv64":
    if len(libLinuxRISCV64) == 0 {
        return nil, fmt.Errorf("嵌入的库文件为空 (linux_riscv64)。请参考 libs/linux_riscv64/embedded.go")
    }
...
3. **交叉编译**：如果使用交叉编译，确保 RISC-V 工具链配置正确
4. **测试验证**：添加支持后，务必在 Linux RISCV64 平台上进行完整测试
