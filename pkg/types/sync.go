...
const (
	// SyncStatusIdle 空闲状态
	// 服务已启动但当前没有进行同步操作（仅用于本地高度≈网络高度时）
	...
	// SyncStatusBootstrapping 启动引导中
	// 节点刚启动或本地高度为0，正在等待或准备执行首次同步
	SyncStatusBootstrapping

	// SyncStatusDegraded 降级状态
	// 网络高度明显高于本地高度，且当前没有活跃同步任务
	SyncStatusDegraded
)
