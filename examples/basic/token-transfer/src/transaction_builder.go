...
func (tb *TransactionBuilder) SignTransaction(tx *Transaction, privateKey string) (*Transaction, error) {
	// 计算交易哈希
...
func (tb *TransactionBuilder) Estimate执行费用(tx *Transaction) (uint64, error) {
	// 基础执行费用费用
	...
	return estimated执行费用, nil
}

// SetExecutionFeePrice 设置执行费用价格
func (tb *TransactionBuilder) SetExecutionFeePrice(ExecutionFeePrice uint64) {
	tb.ExecutionFeePrice = ExecutionFeePrice
}

// SetExecutionFeeLimit 设置执行费用限制
func (tb *TransactionBuilder) SetExecutionFeeLimit(ExecutionFeeLimit uint64) {
...
func DemoTransactionBuilder() {
	...
	// 4. 签名交易
	fmt.Println("4. 签名交易...")
	signedTx, err := builder.SignTransaction(tx, "alice_private_key")
	if err != nil {
		fmt.Printf("签名交易失败: %v\n", err)
		return
	}
	...
	执行费用Estimate, err := builder.Estimate执行费用(tx)
	if err != nil {
		fmt.Printf("估算执行费用失败: %v\n", err)
		return
	}
