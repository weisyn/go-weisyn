...
func (c *StakingClient) Stake(amount *big.Int, lockPeriod uint64) (string, error) {
	...
	// 🚀 步骤3：调用系统质押合约
	// 在真实实现中，这里会调用区块链交易
	txHash := c.callContract("stake", paramsBytes)
...
func (c *StakingClient) Unstake(stakeID string) (string, error) {
	...
	// 🚀 步骤2：调用取消质押
	txHash := c.callContract("unstake", paramsBytes)
...
// 私有方法：调用合约（写操作）
func (c *StakingClient) callContract(method string, params []byte) string {
	// 🔧 在真实实现中，这里会：
	// 1. 构建交易参数
	// 2. 签名交易
	// 3. 发送到WES网络
...
func DemoStakingFlow() {
	...
	// 遵循 WES 地址规范，使用 Base58Check 格式地址
	client := NewStakingClient(
		"CJ89RzBaa2SrLRUbGFY2SFfsu6UMAgqfNZ", // 系统质押合约地址（Base58Check）
		"CY8JpYU6CLAwg3M9yuQM8v1aCJWnSjVEwW", // 用户地址（Base58Check）
	)
	fmt.Println()
