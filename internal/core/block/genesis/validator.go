...
func ValidateBlock(
	...
	// 验证Merkle根（使用统一交易哈希服务 + Merkle树）
	valid, err := validateMerkleRoot(ctx, genesisBlock.Body.Transactions, genesisBlock.Header.MerkleRoot, txHashClient, hashManager, logger)
	if err != nil {
...
func validateMerkleRoot(
	...
	if hashManager == nil {
		return false, fmt.Errorf("哈希管理器不能为空")
	}
	...
	calculatedRoot, err := buildMerkleTree(hasher, txHashes)
	if err != nil {
		return false, fmt.Errorf("计算Merkle根失败: %w", err)
	}
