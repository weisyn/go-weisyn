...
type SyncEventHandler struct {
	...
	// 统计信息（并发安全）
	forkDetectedCount   atomic.Uint64 // 分叉检测次数
	forkProcessingCount atomic.Uint64 // 分叉处理次数
	forkCompletedCount  atomic.Uint64 // 分叉完成次数
	networkChangedCount atomic.Uint64 // 网络质量变化次数

	lastMu sync.RWMutex
	last   SyncEventStatsSnapshot // 最近一次事件的快照（便于观测/诊断）
}

// SyncEventStatsSnapshot 为 SyncEventHandler 的可观测统计快照（用于日志/指标/调试）。
// 注意：仅承载 sync 模块事件层面的信息，不做共识层状态裁决。
type SyncEventStatsSnapshot struct {
	UpdatedAt time.Time

	LastForkDetectedHeight uint64
	LastForkConflictType   string
	LastLocalBlockHash     string
	LastForkBlockHash      string

	LastForkProcessingHeight   uint64
	LastForkProcessingProgress uint32
	LastForkProcessingStatus   string

	LastForkCompletedHeight  uint64
	LastForkCompletedSuccess bool
	LastForkCompletedDurationMs uint64

	LastNetworkQuality       string
	LastNetworkLatencyMs     uint64
	LastNetworkBandwidthBps  uint64
	LastNetworkConnectedPeers uint64
}

// Snapshot 返回当前统计信息快照（线程安全）。
func (h *SyncEventHandler) Snapshot() (counts struct {
	ForkDetected   uint64
	ForkProcessing uint64
	ForkCompleted  uint64
	NetworkChanged uint64
}, last SyncEventStatsSnapshot) {
	counts.ForkDetected = h.forkDetectedCount.Load()
	counts.ForkProcessing = h.forkProcessingCount.Load()
	counts.ForkCompleted = h.forkCompletedCount.Load()
	counts.NetworkChanged = h.networkChangedCount.Load()
...
func (h *SyncEventHandler) HandleForkDetected(eventData *types.ForkDetectedEventData) error {
	h.forkDetectedCount.Add(1)

	h.lastMu.Lock()
	h.last.UpdatedAt = time.Now()
	h.last.LastForkDetectedHeight = eventData.Height
	h.last.LastForkConflictType = eventData.ConflictType
	h.last.LastLocalBlockHash = eventData.LocalBlockHash
	h.last.LastForkBlockHash = eventData.ForkBlockHash
	h.lastMu.Unlock()
...
func (h *SyncEventHandler) HandleForkProcessing(eventData *types.ForkProcessingEventData) error {
	h.forkProcessingCount.Add(1)

	h.lastMu.Lock()
	h.last.UpdatedAt = time.Now()
	h.last.LastForkProcessingHeight = eventData.Height
	h.last.LastForkProcessingProgress = uint32(eventData.Progress)
	h.last.LastForkProcessingStatus = eventData.Status
	h.lastMu.Unlock()
...
func (h *SyncEventHandler) HandleForkCompleted(eventData *types.ForkCompletedEventData) error {
	h.forkCompletedCount.Add(1)

	h.lastMu.Lock()
	h.last.UpdatedAt = time.Now()
	h.last.LastForkCompletedHeight = eventData.FinalHeight
	h.last.LastForkCompletedSuccess = eventData.Success
	h.last.LastForkCompletedDurationMs = uint64(eventData.Duration)
	h.lastMu.Unlock()
...
func (h *SyncEventHandler) HandleNetworkQualityChanged(eventData *types.NetworkQualityChangedEventData) error {
	h.networkChangedCount.Add(1)

	h.lastMu.Lock()
	h.last.UpdatedAt = time.Now()
	h.last.LastNetworkQuality = eventData.Quality
	h.last.LastNetworkLatencyMs = uint64(eventData.Latency)
	h.last.LastNetworkBandwidthBps = uint64(eventData.Bandwidth)
	h.last.LastNetworkConnectedPeers = uint64(eventData.ConnectedPeers)
	h.lastMu.Unlock()
...
func (h *SyncEventHandler) GetSyncEventStats() map[string]interface{} {
	forkDetected := h.forkDetectedCount.Load()
	forkProcessing := h.forkProcessingCount.Load()
	forkCompleted := h.forkCompletedCount.Load()
	networkChanged := h.networkChangedCount.Load()

	return map[string]interface{}{
		"fork_detected_count":    forkDetected,
		"fork_processing_count":  forkProcessing,
		"fork_completed_count":   forkCompleted,
		"network_changed_count":  networkChanged,
		"total_events_processed": forkDetected + forkProcessing + forkCompleted + networkChanged,
	}
}
