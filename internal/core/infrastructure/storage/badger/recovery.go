...
func (s *Store) TryRepair(dataDir string) error {
	...
	opts.ReadOnly = true
	opts.Logger = newBadgerLogger(s.logger, dataDir)
...
func (s *Store) RestoreFromBackup(ctx context.Context, backupPath string, targetDir string) error {
	...
	if _, err := os.Stat(filepath.Join(targetDir, "MANIFEST")); err == nil {
		// 目标目录已存在数据库文件，创建备份
		// 统一归口到 recovery/ 子目录
		recoveryDir := filepath.Join(targetDir, "recovery")
		backupTs := time.Now().Format("20060102_150405")
		existingBackupDir := filepath.Join(recoveryDir, fmt.Sprintf("existing_backup_%s", backupTs))

		s.logger.Infof("目标目录已存在数据库，创建备份: %s", existingBackupDir)

		if err := os.MkdirAll(existingBackupDir, 0700); err != nil {
	...
	if err := os.WriteFile(restoreInfoPath, []byte(restoreInfo), 0600); err != nil {
	...
	opts.Logger = newBadgerLogger(s.logger, targetDir)
