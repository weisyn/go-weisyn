...
func setupTestStore(t *testing.T) (*Store, string, func()) {
	// 确保单测使用磁盘模式，避免环境变量导致意外回退到内存DB（会缺少 MANIFEST 等文件）
	t.Setenv("WES_MEMORY_ONLY_MODE", "")
	t.Setenv("WES_ALLOW_BADGER_FALLBACK_TO_MEMORY", "")
	...
	options := &badgerconfig.BadgerOptions{
		Path:       tempDir,
		SyncWrites: false,
		// BadgerDB 要求 ValueThreshold <= MaxBatchSize；MemTableSize 过小会触发打开失败。
		MemTableSize:         128 << 20, // 128MB（与默认值一致，避免 Badger 参数校验失败）
		EnableAutoCompaction: false,
	}
	cfg := badgerconfig.NewFromOptions(options)
	...
	// 返回清理函数
	cleanup := func() {
		// 注意：BadgerStore 仍包含 Close()，这里优先显式关闭，避免文件锁影响删除临时目录。
		_ = store.Close()
...
func TestNoImplicitFallbackWhenDBLocked(t *testing.T) {
	t.Setenv("WES_MEMORY_ONLY_MODE", "")
	t.Setenv("WES_ALLOW_BADGER_FALLBACK_TO_MEMORY", "")

	tempDir, err := os.MkdirTemp("", "badger-open-fail-test")
	require.NoError(t, err)
	defer func() {
	...
	options := &badgerconfig.BadgerOptions{
		Path:       tempDir,
		SyncWrites: false,
		// 故意给一个 Badger 会拒绝的参数，稳定触发 badger.Open 失败，
		// 用于验证“默认 fail-fast，不隐式回退内存DB”。
		MemTableSize:         1,
		EnableAutoCompaction: false,
	}
	cfg := badgerconfig.NewFromOptions(options)
...
func TestSafeOpenDB_WithBadgerFatal_PerformsSelfHealAndClearsMarker(t *testing.T) {
	// Arrange
	t.Setenv("WES_MEMORY_ONLY_MODE", "")
	t.Setenv("WES_ALLOW_BADGER_FALLBACK_TO_MEMORY", "")

	tempDir, err := os.MkdirTemp("", "badger-fatal-heal-test")
	require.NoError(t, err)
	defer func() { _ = os.RemoveAll(tempDir) }()

	logger := &mockLogger{}

	// 先创建一个可正常打开/关闭的 DB（保证目录结构完整）
	opts := badgerdb.DefaultOptions(tempDir)
	opts.SyncWrites = false
	opts.MemTableSize = 128 << 20
	opts.Logger = newBadgerLogger(logger, tempDir)
	db, err := badgerdb.Open(opts)
	require.NoError(t, err)
	require.NoError(t, db.Close())

	// 写入 BADGER_FATAL 标记，模拟上次运行触发致命前兆
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, "BADGER_FATAL"), []byte("fatal"), 0600))

	// Act
	db2, err := safeOpenDB(tempDir, opts, logger)
	require.NoError(t, err)
	require.NotNil(t, db2)
	require.NoError(t, db2.Close())

	// Assert：成功打开后应清理 BADGER_FATAL
	_, statErr := os.Stat(filepath.Join(tempDir, "BADGER_FATAL"))
	assert.True(t, os.IsNotExist(statErr), "BADGER_FATAL should be cleared after successful open")
}
