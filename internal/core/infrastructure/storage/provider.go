...
// CollectMemoryStats 收集存储模块的内存统计信息（实现 MemoryReporter 接口）
//
// 映射规则（根据 memory-standards.md）：
// - Objects: 打开的存储实例数（BadgerDB + MemoryStore + FileStore + TempStore）
// - ApproxBytes: 存储相关内存估算（主要是 MemoryStore 的缓存）
// - CacheItems: LRU 缓存条数（MemoryStore 的缓存条目）
// - QueueLength: 待处理存储操作队列长度（当前暂为 0，因为存储操作是同步的）
func (p *Provider) CollectMemoryStats() metricsiface.ModuleMemoryStats {
	p.mu.RLock()
	defer p.mu.RUnlock()

	// 统计存储实例数量
	objects := int64(len(p.badgerStores) + len(p.memoryStores) + len(p.fileStores) + len(p.tempStores))

	// 尝试从 MemoryStore / BadgerStore 中获取更贴近真实的内存使用信息。
	// 设计原则：
	// - 优先使用实际统计接口（如 MemoryProvider.GetMemoryUsage）
	// - 当前实现：仅使用 MemoryStore.Count() 作为 cacheItems 的来源，不对 Badger 进行拍脑袋 bytes 估算
	approxBytes := int64(0)
	cacheItems := int64(0)

	// 1) MemoryStore：如果实现了 MemoryProvider（带 GetMemoryUsage/Count），使用其统计
	for _, ms := range p.memoryStores {
		// 动态断言到带 Count() 的接口（MemoryStore 本身就定义了 Count）
		type counter interface {
			Count(ctx context.Context) (int64, error)
		}

		if c, ok := ms.(counter); ok {
			if cnt, err := c.Count(context.Background()); err == nil {
				cacheItems += cnt
			}
		}
	}
