...
func (r *EventRouter) Stop() error {
	...
	// 关闭优先级队列
	for _, queue := range r.priorityQueues {
		close(queue)
	}
...
func (r *EventRouter) processRouteTask(task *RouteTask) error {
	...
	strategy := r.GetRouteStrategy(task.EventType)
	r.mu.RUnlock()
...
func (r *EventRouter) invokeHandler(subscription *SubscriptionInfo, task *RouteTask) error {
	...
	// TODO: 这里需要根据具体的处理器类型进行调用
	// 目前先记录日志，实际实现时需要类型断言和调用
	r.logger.Debugf("调用事件处理器: subscription=%s, event=%s, component=%s",
		subscription.ID, task.EventType, subscription.Component)
...
func (r *EventRouter) GetAllSubscriptions() map[string][]*SubscriptionInfo {
	...
	for eventType, subscriptions := range r.subscriptions {
		result[eventType] = make([]*SubscriptionInfo, len(subscriptions))
