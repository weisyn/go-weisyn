...
type EventBus struct {
	...
	// ================== 历史记录 ==================
	// 🔧 已移除：eventHistory 导致内存泄漏，且功能已简化不再使用
	historyMu    sync.RWMutex // 历史记录锁（保留用于向后兼容）
...
func (eb *EventBus) UpdateConfig(config *types.EventBusConfig) error {
	if config == nil {
		return fmt.Errorf("event bus config is nil")
	}

	// 记录旧值用于决定是否需要清理订阅状态
	oldEnabled := eb.config.IsEnabled()

	// 将 types.EventBusConfig 映射到 internal/config/event.Config
	if err := eb.config.ApplyRuntimeConfig(config); err != nil {
		return err
	}

	newEnabled := eb.config.IsEnabled()

	// 如果从启用 -> 禁用：清理所有订阅/过滤/拦截，避免“禁用后仍触发”的错觉
	if oldEnabled && !newEnabled {
		// 重置底层 bus（asaskevich/EventBus 不提供强一致的全量枚举/清理接口）
		eb.bus = evbus.New()

		eb.weisynMutex.Lock()
		eb.weisynSubscriptions = make(map[string]*weisynSubscription)
		eb.weisynMutex.Unlock()

		eb.filterMutex.Lock()
		eb.filters = make([]event.EventFilter, 0)
		eb.interceptors = make([]event.EventInterceptor, 0)
		eb.filterMutex.Unlock()
	}
...
func (eb *EventBus) ModuleName() string {
	return "event"
}

// CollectMemoryStats 收集事件总线的内存统计信息（实现 MemoryReporter 接口）
//
// 映射规则（根据 memory-standards.md）：
// - Objects: 注册的 subscriber 数（WES 订阅 + 基础订阅估算）
// - ApproxBytes: 事件系统内存估算（订阅映射、事件历史、指标等）
// - CacheItems: 事件缓存条目（如果有）
// - QueueLength: 事件缓冲队列长度（异步通道的缓冲区长度）
func (eb *EventBus) CollectMemoryStats() metricsiface.ModuleMemoryStats {
	// 统计 WES 订阅数量
	eb.weisynMutex.RLock()
	weisynSubCount := len(eb.weisynSubscriptions)
	eb.weisynMutex.RUnlock()
	objects := int64(weisynSubCount)

	// 📌 基础订阅（第三方 EventBus）当前无法精确统计，这里不再做“数量 * 2”的放大估算，
	// 也不再对 bytes 做拍脑袋估算，以免误导。
	approxBytes := int64(0)
	cacheItems := int64(0) // 已移除 eventHistory

	// 队列长度：异步事件通道的缓冲区长度（估算）
	// asaskevich/EventBus 的异步订阅使用 channel，默认缓冲区可能较小
	queueLength := int64(0) // 无法直接获取，设为 0
