...
- 管理路由表的动态更新和维护
- **🆕 节点健康状态管理（抗网络震荡）**
  - 实现4状态节点生命周期：Active → Suspect → Quarantined → Evicted
  - 健康分指数衰减机制（半衰期5分钟）
  - 温和驱逐策略：失败累计而非立即删除
  - Replacement Cache：避免K桶被快速掏空
  - 自动故障恢复：成功交互立即恢复Active状态
...
- **internal/core/network**: 网络模块 - P2P网络通信和节点管理（连接建立、路由维护）
- **internal/core/p2p**: P2P模块 - 管理节点运行时状态（包括在线状态），提供 `RuntimeState` 接口
...
### ⚠️ K桶节点过早驱逐导致震荡（2025-10-08发现并修复）

**问题描述**：
　　每次启动挖矿时，K桶中的节点可能已经下线导致区块发送失败。初步分析发现"立即驱逐"策略在网络震荡场景下会导致K桶被快速掏空——节点暂时离线被驱逐后，没有路径重新加入K桶（因为首次发现已执行过），最终导致K桶为空，无法发送区块。

**根本原因**：

- **缺乏断连处理**：节点断连时未发布事件或未订阅，K桶无法感知节点离线
- **aggressive eviction**：业务失败（发送失败、协议检查失败）立即调用`RemovePeer`删除节点
- **无恢复路径**：被驱逐节点无法重新进入K桶（discovery不会重复发现已知节点）
- **缺少健康分机制**：无状态管理，无法区分"暂时离线"与"永久失效"

**修复方案**（已实施）：

1. **实现节点健康状态机**：
   - 增加4状态生命周期：`Active` → `Suspect` → `Quarantined` → `Evicted`
   - PeerInfo新增字段：`failureCount`, `healthScore`, `quarantinedUntil`, `lastFailureAt`, `peerState`
   - 失败累计机制：连续失败3次才从Active降级为Suspect，累计达阈值进入Quarantined

2. **健康分指数衰减**（2025-12-16修复）：
   - 使用正确的指数衰减公式：`failure(t) = failure_0 * 0.5^(t/halfLife)`
   - 基于Δt维护：新增`lastHealthUpdateAt`字段，只在时间变化时更新
   - 历史失败影响随时间自然减弱，避免"一次失败终身惩罚"
   - 成功交互立即恢复满分（100），快速恢复Active状态

3. **温和驱逐策略**（2025-12-16加强）：
   - **硬约束**：所有清理路径必须先检查`Connectedness != Connected`
   - 仅在满足以下条件时才真正调用`RemovePeer`：
     * **断连**（Connectedness != Connected）【必须】
     * 长期无用（LastUsefulAt超过usefulnessGracePeriod*3）
     * 低健康分（healthScore < 20）或Evicted状态
     * 桶有余量（len(bucket) > minPeersPerBucket）
   - Suspect/Quarantined清理阈值从3/5分钟提升到10分钟（更保守）
   - 增加Replacement Cache（每桶5个替换位），避免桶被掏空

4. **事件驱动架构**（2025-12-16完善）：
   - 实现`networkEventNotifiee`监听libp2p连接/断连事件
   - K桶订阅`EventTypeNetworkPeerConnected`：连接时更新`LastUsefulAt`（自动续期）
   - K桶订阅`EventTypeNetworkPeerDisconnected`：断连时标记Suspect而非删除
   - 维护循环中，已连接peer自动续期`LastUsefulAt`，避免误判为"长期无用"
   - 业务层（交易传播、聚合器转发）调用`RecordPeerFailure`累计失败分

5. **智能节点选择**：
   - `FindClosestPeers`自动过滤：Quarantined节点、低健康分节点（<0.3）、未连接节点
   - 保留10%灰度探测位给Suspect节点，允许临时故障节点快速恢复
   - 在`selectKBucketPeersForSync`、`broadcastViaStreamRPC`、`forwardBlock`中集成健康记录

6. **维护协程**（2025-12-16调整）：
   - 周期性（30秒，从10秒调整）执行健康分衰减、隔离解除、清理驱逐
   - 对Quarantined节点执行探测（10秒间隔），成功则恢复Active
   - 新增可观测指标：记录清理原因、状态分布、违规清理计数等

7. **配置调整**（2025-12-16更新）：
   - `UsefulnessGracePeriod`：30秒 → 60秒（更保守）
   - `MinPeersPerBucket`：1 → 2（增强桶稳定性）
   - `MaintainInterval`：10秒 → 30秒（降低维护频率）
   - `HealthDecayHalfLife`：保持5分钟（但公式修正为真正的指数衰减）

**验收标准**：

- ✅ 节点断连后标记为Suspect，不立即从K桶删除
- ✅ 发送失败累计3次才降级，单次失败不驱逐
- ✅ 健康分随时间自然衰减，历史失败影响减弱
- ✅ 成功交互后立即恢复Active状态
- ✅ 网络震荡场景下K桶不被掏空（保持≥minPeersPerBucket个健康节点）
- ✅ FindClosestPeers只返回健康、已连接的节点
- ✅ 所有修改通过编译和linter检查
- 🔜 集成测试：模拟节点反复上下线，验证K桶稳定性（待执行）

**影响范围**：

- `pkg/types/kademlia.go`: PeerInfo新增健康管理字段
- `pkg/interfaces/infrastructure/kademlia/kbucket.go`: RoutingTableManager新增`RecordPeerSuccess/Failure`接口
- `internal/core/infrastructure/kademlia/`: bucket.go, manager.go, config.go 实现健康管理逻辑
- `internal/core/infrastructure/node/impl/host/network_notifiee.go`: 新增网络事件通知器
- `internal/core/infrastructure/node/module.go`: 注册网络事件通知器
- `internal/core/infrastructure/kademlia/module.go`: 订阅断连事件
- `internal/core/blockchain/sync/node_selection.go`: 同步选择注释健康过滤
- `internal/core/tx/transaction/submit/helpers.go`: 交易传播增加失败记录
- `internal/core/consensus/aggregator/controller/start_aggregation.go`: 聚合器转发增加失败记录
...
- **状态机管理**: 4状态节点生命周期（Active/Suspect/Quarantined/Evicted）
- **失败容忍**: 允许临时失败，累计达阈值才降级（默认3次）
- **健康分衰减**: 指数衰减机制自动减弱历史失败影响（半衰期5分钟）
- **温和驱逐**: 仅在长期无用+低健康+桶有余量时才移除节点
- **快速恢复**: 成功交互立即恢复Active状态和满健康分
- **智能选择**: FindClosestPeers自动过滤Quarantined和低健康分节点
- **替换缓存**: 每桶5个替换位，避免K桶被快速掏空
- **事件驱动**: 订阅peer断连事件，自动标记Suspect而非立即删除
...
kademlia:
  ...
  # 🆕 节点健康管理配置 (抗网络震荡)
  health_management:
    failure_threshold: 3            # 失败阈值：累计失败次数超过此值进入Suspect
    health_score_threshold: 0.3     # 健康分阈值：低于此值被过滤（0.0-1.0）
    quarantine_duration: "5m"       # 隔离时长：Quarantined状态持续时间
    health_decay_halflife: "5m"     # 健康分半衰期：失败影响的衰减周期
    maintenance_interval: "30s"     # 维护周期：健康分衰减和隔离解除检查间隔
    min_peers_per_bucket: 5         # 最小桶容量：低于此值不主动驱逐节点
    probe_interval: "10s"           # 探测周期：对隔离节点的探测间隔
    max_replacement_cache_size: 5   # 替换缓存大小：每桶最多缓存的替换节点数
    enable_gentle_eviction: true    # 温和驱逐：启用抗震荡的健康管理机制
  
  # 健康检查（传统Ping机制，与健康管理互补）
