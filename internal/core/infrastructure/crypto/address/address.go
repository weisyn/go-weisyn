...
var (
	...
	// ErrETHAddressNotSupported ETH 地址格式不受支持
	ErrETHAddressNotSupported = errors.New("ETH-style 0x-prefixed addresses are not supported; WES uses Base58Check format")
)
...
func base58CheckDecode(encoded string) ([]byte, byte, error) {
	...
	if err != nil {
		return nil, 0, fmt.Errorf("base58 decode failed: %w", err)
	}
	if len(decoded) < 5 {
...
// IsETHStyleAddress 检查是否为 ETH 风格的 0x 前缀地址
//
// WES 不支持 ETH 风格地址，所有地址必须使用 Base58Check 格式。
// 此函数用于提前检测并拒绝 ETH 格式的输入。
func IsETHStyleAddress(address string) bool {
	return len(address) > 2 && (address[:2] == "0x" || address[:2] == "0X")
}

// StringToAddress 解析字符串为标准地址格式
//
// 遵循 WES 地址规范：
// - 只接受 Base58Check 编码的地址
// - 拒绝 ETH 风格的 0x 前缀地址
func (s *AddressService) StringToAddress(addressStr string) (string, error) {
	...
	// 明确拒绝 ETH 风格地址
	if IsETHStyleAddress(addressStr) {
		return "", ErrETHAddressNotSupported
	}

	// 只接受 Bitcoin风格地址
	valid, err := s.ValidateAddress(addressStr)
	if !valid || err != nil {
		return "", fmt.Errorf("invalid WES address: %w", err)
	}
