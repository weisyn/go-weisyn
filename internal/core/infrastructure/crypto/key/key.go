...
func (p *PrivateKeyPool) secureWipe(data []byte) {
	...
	if _, err := rand.Read(randomData); err != nil {
		...
		hasher.Write(data)
		hasher.Write([]byte(time.Now().String()))
		copy(randomData, hasher.Sum(nil))
	}
...
func (km *KeyManager) collectEntropyPeriodically() {
	...
	for range ticker.C {
		...
		if _, err := rand.Read(extraEntropy); err != nil {
			...
			hasher.Write(km.entropyPool)
			hasher.Write(timeBytes)
			copy(extraEntropy, hasher.Sum(nil))
		}
...
func (km *KeyManager) GenerateKeyPairWithContext(ctx context.Context) ([]byte, []byte, error) {
	...
	// 使用secp256k1曲线生成私钥
	privateKey, err := ecdsa.GenerateKey(km.secp256k1Curve.S256(), reader)
	if err != nil {
		return nil, nil, err
	}
...
func (km *KeyManager) DerivePublicKey(privateKey []byte) ([]byte, error) {
	...
	// 计算公钥点
	x, y := km.secp256k1Curve.S256().ScalarBaseMult(k.Bytes())
	if x == nil || y == nil {
		return nil, ErrInvalidPrivateKey
	}
...
func (km *KeyManager) DerivePublicKeyWithContext(ctx context.Context, privateKey []byte) ([]byte, error) {
	...
	// 获取公钥字节（去掉前缀字节）
	pubBytes := elliptic.Marshal(km.secp256k1Curve.S256(), x, y)[1:]
	copy(publicKeyBytes, pubBytes)
...
func (km *KeyManager) PrivateKeyToECDSA(privateKey []byte) (*ecdsa.PrivateKey, error) {
	...
	priv.D = k
	priv.Curve = km.secp256k1Curve.S256()
...
// bytesToECDSA 将65字节未压缩公钥转换为ECDSA公钥
// 替代 go-ethereum 的 crypto.UnmarshalPubkey
func (km *KeyManager) bytesToECDSA(pubKeyBytes []byte) (*ecdsa.PublicKey, error) {
	if len(pubKeyBytes) != 65 {
		return nil, fmt.Errorf("无效的公钥长度: %d，期望65字节", len(pubKeyBytes))
	}

	if pubKeyBytes[0] != 0x04 {
		return nil, fmt.Errorf("无效的公钥前缀: 0x%02x，期望0x04", pubKeyBytes[0])
	}
...
func (km *KeyManager) compressedPublicKeyToECDSA(compressedKey []byte) (*ecdsa.PublicKey, error) {
	...
	// 转换为ECDSA公钥
	return km.bytesToECDSA(uncompressedKey)
}
...
func (km *KeyManager) uncompressed64PublicKeyToECDSA(publicKey []byte) (*ecdsa.PublicKey, error) {
	...
	return km.bytesToECDSA(pubKeyBytes)
}
...
func (km *KeyManager) GenerateCompressedKeyPair() ([]byte, []byte, error) {
	// 生成ECDSA私钥
	privateKey, err := ecdsa.GenerateKey(km.secp256k1Curve.S256(), rand.Reader)
	if err != nil {
		return nil, nil, err
	}
...
func (km *KeyManager) ValidatePrivateKey(privateKey []byte) error {
	...
	curveOrder := km.secp256k1Curve.S256().Params().N
...
func (km *KeyManager) validateUncompressedPublicKey(publicKey []byte) error {
	...
	// 验证点是否在secp256k1曲线上
	if !km.secp256k1Curve.S256().IsOnCurve(x, y) {
