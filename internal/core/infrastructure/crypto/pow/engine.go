...
func NewEngine(hashManager crypto.HashManager, logger log.Logger, config *consensusconfig.POWConfig) (*Engine, error) {
	...
	if config == nil {
		config = &consensusconfig.POWConfig{
			...
			// v2 参数（用于内部难度计算器/限幅；不参与链上确定性校验）
			MaxAdjustUpPPM:        4_000_000, // 允许4倍上调
			MaxAdjustDownPPM:      250_000,   // 允许降至 1/4
			EMAAlphaPPM:           0,         // 默认禁用 EMA（内部用途）
			MTPWindow:             11,
...
func (e *Engine) MineBlockHeader(ctx context.Context, header *core.BlockHeader) (*core.BlockHeader, error) {
	...
	if err != nil {
		// v2：slice mining 会频繁触发 ctx deadline/cancel，这是“预期控制流”，不应记录为 ERROR。
		// 真实错误（参数/序列化/内部异常等）仍然记录为 ERROR。
		switch {
		case errors.Is(err, context.DeadlineExceeded):
			e.logger.Infof("挖矿本轮尝试超时（将由上层重建候选并重试）: height=%d difficulty=%d err=%v",
				header.GetHeight(), header.GetDifficulty(), err)
		case errors.Is(err, context.Canceled):
			e.logger.Debugf("挖矿被取消: height=%d difficulty=%d err=%v",
				header.GetHeight(), header.GetDifficulty(), err)
		default:
