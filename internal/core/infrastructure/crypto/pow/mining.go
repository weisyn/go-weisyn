...
func (m *MiningEngine) MineBlockHeader(ctx context.Context, header *core.BlockHeader) (*core.BlockHeader, error) {
	...
	for {
		...
		// ⚠️ 共识关键：禁止在挖矿循环中修改 Timestamp。
		//
		// 背景：
		// - v2 共识中，区块难度 expectedDifficulty 绑定 block.Header.Timestamp（NextDifficultyForTimestamp）。
		// - 如果 PoW 引擎在挖矿过程中滚动更新 minedHeader.Timestamp，但不同步重算 minedHeader.Difficulty，
		//   则会产生“区块难度不匹配 got!=expected”的共识裂缝（同一节点也可能自我拒绝）。
		//
		// 正确做法：
		// - Timestamp 应由候选块构建阶段一次性确定（并与 Difficulty 同步计算），在 PoW nonce 搜索期间保持不变；
		// - 如需“挖太久刷新时间戳”，应在更高层（挖矿编排/候选重建）重新构建候选块（新的 Timestamp + 新的 Difficulty），而不是在此处偷偷修改字段。
		...
		// 周期性进度日志
		// 🔧 修复：创世区块（高度=0）使用更频繁的日志，避免用户以为系统卡住
		logInterval := uint64(1000000) // 默认：每100万次尝试
		if minedHeader.Height == 0 {
			logInterval = 10000 // 创世区块：每1万次尝试输出一次（约每1-2秒）
		}

		if attempts%logInterval == 0 {
			elapsed := time.Since(startTime)
			progressElapsed := time.Since(lastProgressLog)
			intervalAttempts := logInterval
			if attempts == 0 {
				intervalAttempts = 1
			}
			currentHashRate := float64(intervalAttempts) / progressElapsed.Seconds()
			totalHashRate := float64(attempts) / elapsed.Seconds()

			if minedHeader.Height == 0 {
				logger.Infof("⛏️  创世区块挖矿进度: 尝试=%d次, 耗时=%v, 算力=%.2f H/s",
					attempts, elapsed, totalHashRate)
			} else {
