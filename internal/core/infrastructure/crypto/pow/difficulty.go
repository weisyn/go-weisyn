...
func (d *DifficultyCalculator) CalculateNextDifficulty(ctx context.Context,
	...
	// 记录难度历史高度：使用 recentBlocks 推导 next height，避免 Height=0 的误导统计
	var nextHeight uint64
	if n := len(recentBlocks); n > 0 && recentBlocks[n-1] != nil {
		nextHeight = recentBlocks[n-1].Height + 1
	}
	d.recordDifficultyHistory(nextHeight, currentDifficulty, newDifficulty, adjustmentRatio)
...
func (d *DifficultyCalculator) applyAdjustmentLimits(currentDifficulty, newDifficulty uint64) uint64 {
	config := d.coreEngine.GetConfig()
	// v2：使用 PPM 表示的上下限（内部限幅；不参与链上确定性校验）
	upPPM := config.MaxAdjustUpPPM
	downPPM := config.MaxAdjustDownPPM
	if upPPM < 1_000_000 {
		upPPM = 4_000_000
	}
	if downPPM == 0 || downPPM > 1_000_000 {
		downPPM = 250_000
	}

	upFactor := float64(upPPM) / 1_000_000.0
	downFactor := float64(downPPM) / 1_000_000.0
	...
	// 检查是否超过上限
	if ratio > upFactor {
		limited := uint64(float64(currentDifficulty) * upFactor)
	...
	// 检查是否低于下限
	if ratio < downFactor {
		limited := uint64(float64(currentDifficulty) * downFactor)
		d.coreEngine.GetLogger().Warnf("难度调整低于下限，限制: %.2f → %.2f",
			ratio, downFactor)
		return limited
	}
...
//   - newDifficulty: 调整后难度
//   - adjustmentRatio: 调整比例
func (d *DifficultyCalculator) recordDifficultyHistory(height uint64, oldDifficulty, newDifficulty uint64, adjustmentRatio float64) {
