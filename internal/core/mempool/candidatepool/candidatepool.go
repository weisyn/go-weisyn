...
// 返回：
// - mempoolIfaces.CandidatePool：候选池实例；
// - error：初始化失败时返回错误。
...
) (mempoolIfaces.CandidatePool, error) {
...
func (p *CandidatePool) AddCandidate(block *core.Block, fromPeer string) ([]byte, error) {
	...
	if _, exists := p.candidates[blockHashStr]; exists {
		p.duplicateBlocks++
	...
	// 存储候选区块
	p.candidates[blockHashStr] = candidate
	p.pendingCandidates[blockHashStr] = struct{}{}
	...
	if p.logger != nil {
		p.logger.Infof("添加候选区块成功，高度: %d, 哈希: %x, 来源: %s, 交易数: %d",
			height, blockHash[:8], fromPeer, len(block.Body.Transactions))
	}
