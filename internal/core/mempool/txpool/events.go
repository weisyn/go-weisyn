...
// 由 event_handler/event_sink.go 注入具体发布逻辑，实现统一对外事件标准化。
package txpool
...
// - OnTxExpired：tx 为已过期的交易包装器（P2-6新增）
// - OnPoolStateChanged：交易池状态变化事件（P2-6新增）
type TxEventSink interface {
	OnTxAdded(tx *TxWrapper)
	OnTxRemoved(tx *TxWrapper)
	OnTxConfirmed(tx *TxWrapper, blockHeight uint64)
	// P2-6: 新增事件类型
	OnTxExpired(tx *TxWrapper)
	OnPoolStateChanged(metrics *PoolMetrics)
}
...
// OnTxExpired 空实现（P2-6新增）。
// 参数：tx 交易包装器。
// 返回：无。
func (NoopTxEventSink) OnTxExpired(tx *TxWrapper) {}

// OnPoolStateChanged 空实现（P2-6新增）。
// 参数：metrics 交易池监控指标。
// 返回：无。
func (NoopTxEventSink) OnPoolStateChanged(metrics *PoolMetrics) {}
