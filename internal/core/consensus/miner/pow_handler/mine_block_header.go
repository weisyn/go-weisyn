...
func (s *PoWComputeService) mineBlockHeader(ctx context.Context, header *core.BlockHeader) (*core.BlockHeader, error) {
	...
	if err != nil {
		// slice mining 场景下，ctx 超时/取消属于预期控制流，避免打 ERROR 噪声
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled) {
			s.logger.Debugf("POW引擎挖矿被取消/超时（将由上层重试）: %v", err)
			return nil, fmt.Errorf("POW引擎挖矿被取消/超时: %w", err)
		}
		s.logger.Errorf("POW引擎挖矿失败: %v", err)
		return nil, fmt.Errorf("POW引擎挖矿失败: %w", err)
	}
