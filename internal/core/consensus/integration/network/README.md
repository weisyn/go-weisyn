...
- **架构边界清晰**：Integration层仅负责协议适配，具体的路由决策、内容寻址、高度校验等共识业务逻辑由Aggregator模块负责
...
```mermaid
graph TB
    subgraph "共识网络集成层架构 (Consensus Network Integration)"
        subgraph "协议注册与管理"
            PROTOCOLS["protocols.go<br/>共识协议常量<br/>版本管理"]
            STREAM_H["stream_handlers.go<br/>流式协议注册<br/>区块提交协议"]
            SUB_H["subscribe_handlers.go<br/>订阅协议注册<br/>共识结果广播"]
        end
        
        subgraph "组件网络处理器 (Target Handlers)"
            MINER_NH["miner/network/handler.go<br/>矿工网络处理器"]
            AGG_NH["aggregator/network/handler.go<br/>聚合器网络处理器"]
        end
    ...
    %% 注意：Miner不直接接收网络消息，而是通过内部接口提交给本地Aggregator
    MINER_NH -.-> AGG_NH
...
- **Integration层**：仅负责协议适配，不包含共识业务逻辑
