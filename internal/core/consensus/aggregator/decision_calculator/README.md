...
【模块定位】
　　本模块是WES PoW+XOR共识架构中**基础验证组件**，在距离选择架构中承担**基础候选验证**功能。复杂的多维度评分算法已被**XOR距离选择算法**替代，本模块现在专注于基础的PoW验证和格式检查。
...
type PoWValidator interface {
    ValidatePoW(candidate *types.CandidateBlock) error
}
```
- **PoW验证**: 验证区块是否满足工作量证明要求
- **难度检查**: 检查区块难度是否符合要求
...
classDiagram
    class DecisionCalculator {
        <<interface>>
        +EvaluateAllCandidates(candidates []CandidateBlock) ([]CandidateBlock, error)
