...
graph TB
    subgraph "聚合器核心架构"
        ...
        subgraph "区块选择层"
            BLOCK_SELECTOR["🎯 区块选择器<br/>Block Selector"]
    ...
    DECISION_CALCULATOR --> BLOCK_SELECTOR
    BLOCK_SELECTOR --> SCORING_ENGINE
    SCORING_ENGINE --> SELECTION_STRATEGY
    SELECTION_STRATEGY --> FINAL_VALIDATOR
    
    BLOCK_SELECTOR --> RESULT_DISTRIBUTOR
    ...
    style BLOCK_SELECTOR fill:#E3F2FD
...
├── 🎯 block_selector/        # 区块选择器：距离选择协调、选择证明生成（简化功能）
│   ├── manager.go            # 实现 BlockSelector 接口
│   ├── select_best_block.go  # 基于距离的最优选择（简化）
│   ├── tie_breaking.go       # 距离tie-breaking处理（备用）
│   └── selection_validation.go # 距离选择证明生成
...
- **证明生成功能**：原ProofGenerator组件功能整合到BlockSelector的selection_validation.go中  
...
var Module = fx.Module("aggregator",
    ...
    fx.Provide(
        ...
        // 区块选择层
        block_selector.NewManager,
        block_selector.NewBlockSelector,
        block_selector.NewScoringEngine,
        block_selector.NewSelectionStrategy,
...
classDiagram
    ...
    BlockSelector <|-- BlockSelectorImpl : implements
    ...
    AggregatorController --> BlockSelector : uses
...
- [区块选择器实现](block_selector/README.md)
