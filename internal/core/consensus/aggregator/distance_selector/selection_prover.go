...
func (p *selectionProver) calculateDistanceSummary(results []types.DistanceResult) []byte {
	...
	for _, result := range results {
		hasher.Write(result.Candidate.BlockHash)
		hasher.Write([]byte(result.Distance.String()))
	}
...
func (p *selectionProver) calculateProofHash(proof *types.DistanceSelectionProof) []byte {
	...
	hasher.Write(proof.SelectedBlockHash)
	hasher.Write(proof.ParentBlockHash)
	hasher.Write([]byte(proof.SelectedDistance))
	hasher.Write(proof.DistanceSummary)
	hasher.Write([]byte(proof.Algorithm))

	if proof.TieBreakingApplied && proof.TieBreakingProof != nil {
		hasher.Write(proof.TieBreakingProof.SelectedBlockHash)
		hasher.Write([]byte(proof.TieBreakingProof.BreakingStrategy))
	}
