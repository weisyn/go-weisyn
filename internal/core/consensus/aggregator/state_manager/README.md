...
【模块定位】
　　本模块是WES PoW+XOR共识架构中聚合器状态机的核心实现，负责管理聚合器在PoW+XOR共识流程中的8个核心状态转换。作为聚合器内部协调的中枢组件，通过严格的状态转换控制和一致性验证，确保聚合器各个业务组件的有序协作，支撑PoW+XOR架构中"按需激活，任务完成即结束"的生命周期模式。
...
```mermaid
graph TB
    subgraph "聚合器状态管理器架构设计"
        subgraph "状态定义层"
            STATE1["空闲状态<br/>😴 Idle"]
            STATE2["监听状态<br/>👂 Listening"] 
            STATE3["收集状态<br/>📥 Collecting"]
            STATE4["评估状态<br/>🔍 Evaluating"]
            STATE5["选择状态<br/>🎯 Selecting"]
            STATE6["分发状态<br/>📡 Distributing"]
            STATE7["暂停状态<br/>⏸️ Paused"]
            STATE8["错误状态<br/>❌ Error"]
        end
        
        subgraph "转换控制层"
            TRANS1["转换验证<br/>✅ Transition Validation"]
            TRANS2["转换执行<br/>⚙️ Transition Execution"]
            TRANS3["转换确认<br/>✔️ Transition Confirmation"]
        end
        
        subgraph "并发控制层"
            SYNC1["状态锁管理<br/>🔒 State Lock"]
            SYNC2["原子操作<br/>⚛️ Atomic Operation"]
            SYNC3["一致性检查<br/>🔍 Consistency Check"]
        end
        
        subgraph "服务接口层"
            API1["状态查询<br/>📋 State Query"]
            API2["状态设置<br/>⚙️ State Setting"]
            API3["状态历史<br/>📜 State History"]
        end
        
        subgraph "基础设施层"
            INFRA1["Mutex<br/>🔒 互斥锁"]
            INFRA2["Logger<br/>📝 日志记录"]
            INFRA3["Metrics<br/>📊 状态指标"]
        end
    end
...
flowchart TD
    subgraph "统一状态转换实现架构"
        subgraph "状态请求层"
            A[状态操作请求] --> B{操作类型}
            B -->|状态查询| C[GetCurrentState]
            B -->|状态设置| D[SetAggregationState]
            B -->|历史查询| E[GetStateHistory]
        end
