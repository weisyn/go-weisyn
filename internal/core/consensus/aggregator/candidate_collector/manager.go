...
func NewCandidateCollectorService(
	...
	configProvider config.Provider, // ✅ 用于读取 blockchain.block.min_block_interval
) interfaces.CandidateCollector {
	minBlockIntervalSeconds := uint64(0)
	if configProvider != nil {
		if chainOpts := configProvider.GetBlockchain(); chainOpts != nil {
			if chainOpts.Block.MinBlockInterval > 0 {
	...
	// 创建候选验证器，传入配置避免硬编码
	validator := newCandidateValidator(logger, chainQuery, hashManager, powEngine, syncService, config, minBlockIntervalSeconds)
