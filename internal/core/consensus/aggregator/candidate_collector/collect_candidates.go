...
func (m *collectionManager) collectCandidateFromMempool(height uint64) error {
	...
	for i := range candidates {
		if err := m.processCandidateBlock(height, candidates[i]); err != nil {
			m.logger.Info("处理候选区块失败")
			continue
		}
	}
...
func (m *collectionManager) processCandidateBlock(height uint64, candidate *types.CandidateBlock) error {
	window := m.activeWindows[height]
	if window == nil {
		return errors.New("collection window not found")
	}
