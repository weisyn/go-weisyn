...
func NewAggregatorControllerService(
	...
	// 创建聚合启动器（传入编排所需的组件）
	starter := newAggregationStarter(logger, stateManager, election, candidateCollector, decisionCalculator, blockSelector, distanceSelector, resultDistributor, candidatePool, networkService, host, routingTableManager)
