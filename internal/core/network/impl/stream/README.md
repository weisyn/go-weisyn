# 流式协议模块（internal/core/network/impl/stream）
...
graph TB
    subgraph "Stream Protocol Architecture"
        STREAM_SERVICE["Stream Service<br/>流式服务门面"]
        
        subgraph "Stream Management"
            STREAM_CLIENT["Stream Client<br/>流式客户端"]
            STREAM_DISPATCHER["Stream Dispatcher<br/>流分发器"]
            CONNECTION_MANAGER["Connection Manager<br/>连接管理器"]
            MULTIPLEXER["Stream Multiplexer<br/>流复用器"]
        end
        
        subgraph "Quality Control"
            TIMEOUT_MANAGER["Timeout Manager<br/>超时管理器"]
            RETRY_ENGINE["Retry Engine<br/>重试引擎"]
            BACKPRESSURE_CONTROLLER["Backpressure Controller<br/>背压控制器"]
            FLOW_CONTROLLER["Flow Controller<br/>流量控制器"]
        end
        
        subgraph "Data Processing"
            MESSAGE_CODEC["Message Codec<br/>消息编解码器"]
            COMPRESSION_ENGINE["Compression Engine<br/>压缩引擎"]
            FRAME_PROCESSOR["Frame Processor<br/>帧处理器"]
            BUFFER_MANAGER["Buffer Manager<br/>缓冲管理器"]
        end
        
        subgraph "Stream Options"
            STREAM_OPTIONS["Stream Options<br/>流选项配置"]
            QUALITY_OPTIONS["Quality Options<br/>质量选项"]
            PERFORMANCE_OPTIONS["Performance Options<br/>性能选项"]
            SECURITY_OPTIONS["Security Options<br/>安全选项"]
        end
    end
...
```
internal/core/network/impl/stream/
