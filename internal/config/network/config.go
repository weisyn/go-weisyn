...
type NetworkOptions struct {
	...
	MaxConnectionsPerIP    int           `json:"max_connections_per_ip"`    // 每IP最大连接数
	MaxMessagesPerWindow   int           `json:"max_messages_per_window"`   // 每窗口最大消息数
	MessageRateLimitWindow time.Duration `json:"message_rate_limit_window"` // 消息速率限制窗口
}
...
func createDefaultNetworkOptions() *NetworkOptions {
	return &NetworkOptions{
		...
		MaxMessagesPerWindow:   defaultMaxMessagesPerWindow,
		MessageRateLimitWindow: defaultMessageRateLimitWindow,
	}
}
...
func (c *Config) GetMaxConnections() int {
	return c.options.MaxConnections
}

// GetMaxConnectionsPerIP 获取每IP最大连接数
func (c *Config) GetMaxConnectionsPerIP() int {
	return c.options.MaxConnectionsPerIP
}

// GetMaxMessagesPerWindow 获取每窗口最大消息数
func (c *Config) GetMaxMessagesPerWindow() int {
	return c.options.MaxMessagesPerWindow
}

// GetMessageRateLimitWindow 获取消息速率限制窗口
func (c *Config) GetMessageRateLimitWindow() time.Duration {
	return c.options.MessageRateLimitWindow
}
