// Package network provides default configuration values for network service.
package network
...
const (
	...
	// defaultMaxConnections 默认最大连接数设为1000
	// 原因：1000个连接足够支持中型网络规模，同时避免资源耗尽
	defaultMaxConnections = 1000

	// defaultMaxConnectionsPerIP 默认每IP最大连接数设为50
	// 原因：50个连接per IP能防止单点攻击，同时允许合法的多实例部署
	defaultMaxConnectionsPerIP = 50

	// defaultMaxMessagesPerWindow 默认每窗口最大消息数设为100
	// 原因：100条消息/分钟能防止消息洪水攻击
	defaultMaxMessagesPerWindow = 100

	// defaultMessageRateLimitWindow 默认消息速率限制窗口设为1分钟
	// 原因：1分钟窗口既能平滑短期突发，又能快速响应攻击
	defaultMessageRateLimitWindow = 1 * time.Minute
)
