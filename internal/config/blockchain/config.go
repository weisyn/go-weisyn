...
type ResourceLimitsConfig struct {
	GlobalQuota       uint64 `json:"global_quota"`        // 全局配额
	ExecutionTime     uint64 `json:"execution_time"`      // 执行时间限制
...
func New(userConfig interface{}) *Config {
	...
	if userConfig != nil {
		...
		if os.Getenv("WES_CLI_MODE") != "true" {
			println("🔧 CONFIG DEBUG: 配置处理完成")
		}
	} else {
		if os.Getenv("WES_CLI_MODE") != "true" {
			println("🔧 CONFIG DEBUG: userConfig为nil，使用默认配置")
		}
	}
		...
		if structConfig, ok := userConfig.(*struct {
			...
			if genesisMap, exists := configMap["genesis"]; exists {
				if genesisConfig, ok := genesisMap.(map[string]interface{}); ok {
					// 处理genesis_accounts
					if accountsInterface, exists := genesisConfig["genesis_accounts"]; exists {
								...
								if accountMap, ok := accountInterface.(map[string]interface{}); ok {
									...
									if amount, exists := accountMap["amount"]; exists {
										if amountFloat, ok := amount.(float64); ok {
											println("🔧 DEBUG: JSON解析的原始amount:", fmt.Sprintf("%.0f", amountFloat))
											println("🔧 DEBUG: 转换为uint64:", uint64(amountFloat))
											...
											if amountStr, ok := amount.(string); ok {
												if amountInt, err := strconv.ParseUint(amountStr, 10, 64); err == nil {
													println("🔧 DEBUG: 从字符串解析amount:", amountInt)
...
func (c *Config) GetUnifiedGenesisConfig() *types.GenesisConfig {
	...
	if len(internalConfig.GenesisAccounts) > 0 {
