...
func TestConfigAccessors(t *testing.T) {
	...
	t.Run("增强配置访问方法", func(t *testing.T) {
		...
		assert.True(t, config.IsDomainRegistryEnabled())
		...
		assert.True(t, config.IsEventRouterEnabled())
		...
		assert.True(t, config.IsEventValidatorEnabled())
		...
		assert.True(t, config.IsEventCoordinatorEnabled())
...
func TestDefaultValues(t *testing.T) {
	t.Run("时间相关默认值", func(t *testing.T) {
		...
		assert.Equal(t, 30*time.Second, defaultRouteTimeout)
		assert.Equal(t, 5*time.Second, defaultValidationTimeout)
		assert.Equal(t, 60*time.Second, defaultEventTimeout)
	...
	t.Run("数量相关默认值", func(t *testing.T) {
		...
		assert.Equal(t, 1000, defaultMaxSubscribers)
		assert.Equal(t, 100, defaultMaxDomains)
		assert.Equal(t, 50, defaultMaxConcurrentRoutes)
		assert.Equal(t, 10000, defaultMaxQueueSize)
		assert.Equal(t, 20, defaultRouterWorkerPoolSize)
		assert.Equal(t, 50, defaultMaxValidationRules)
	...
	t.Run("布尔相关默认值", func(t *testing.T) {
		...
		assert.True(t, defaultEnhancedEnabled)
		assert.True(t, defaultDomainRegistryEnabled)
		assert.False(t, defaultStrictDomainCheck) // 开发友好
		assert.True(t, defaultWarnCrossDomain)
		assert.True(t, defaultAllowUnregisteredDomain) // 向后兼容
		assert.True(t, defaultEventRouterEnabled)
		assert.True(t, defaultEnablePriorityQueue)
		assert.True(t, defaultEnableRouterMetrics)
		assert.True(t, defaultEventValidatorEnabled)
		assert.False(t, defaultValidatorStrictMode) // 开发友好
		assert.True(t, defaultValidateEventName)
		assert.True(t, defaultValidateEventData)
		assert.True(t, defaultEnableBatchValidation)
		assert.True(t, defaultCacheValidationResults)
		assert.True(t, defaultEventCoordinatorEnabled)
		assert.True(t, defaultEnableCircuitBreaker)
