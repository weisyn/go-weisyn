...
// ApplyRuntimeConfig 将通用 EventBusConfig 映射到本模块的 EventOptions（运行时更新）。
//
// 说明：
// - legacy EventBus 当前仅使用到 Enabled / BufferSize / MaxWorkers / MaxSubscribers 这几项；
// - 其他字段属于上层/增强能力配置，在此处不做强行映射，避免产生“看起来生效但其实无用”的错觉。
func (c *Config) ApplyRuntimeConfig(cfg *types.EventBusConfig) error {
	if c == nil || c.options == nil {
		return fmt.Errorf("event config is nil")
	}
	if cfg == nil {
		return fmt.Errorf("event bus config is nil")
	}

	// 基础字段映射（保持兼容：优先使用扩展字段 MaxConcurrentEvents/EventQueueSize）
	if cfg.MaxConcurrentEvents > 0 {
		c.options.MaxWorkers = cfg.MaxConcurrentEvents
	} else if cfg.WorkerPoolSize > 0 {
		c.options.MaxWorkers = cfg.WorkerPoolSize
	}

	if cfg.EventQueueSize > 0 {
		c.options.BufferSize = cfg.EventQueueSize
	} else if cfg.BufferSize > 0 {
		c.options.BufferSize = cfg.BufferSize
	}

	if cfg.MaxSubscriptions > 0 {
		c.options.MaxSubscribers = cfg.MaxSubscriptions
	}

	// 启用开关：EventBusConfig 没有显式 Enabled 字段，这里仅在 Metadata 指定时允许更新，避免误关/误开。
	if cfg.Metadata != nil {
		if v, ok := cfg.Metadata["enabled"]; ok {
			if b, ok := v.(bool); ok {
				c.options.Enabled = b
			}
		}
