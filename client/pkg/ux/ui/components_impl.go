...
// NewComponents 创建 UI 组件实例
//
// 参数:
//
//	logger: 日志记录器（可以传入 nil 或 NoopLogger()）
//
// 返回:
//
//	Components 接口实例
func NewComponents(logger Logger) Components {
	...
	// 如果不是TTY，禁用pterm的颜色和交互功能
	if !isTTY {
		pterm.DisableColor()
		pterm.DisableStyling()
		logger.Warn("检测到非TTY环境，已禁用颜色和交互功能")
	} else {
		logger.Debug("检测到TTY环境，启用完整UI功能")
	}
...
// ShowConfirmDialog 显示确认对话框（默认值为No）
func (c *components) ShowConfirmDialog(title, message string) (bool, error) {
	return c.ShowConfirmDialogWithDefault(title, message, false)
}

// ShowConfirmDialogWithDefault 显示确认对话框（可指定默认值）
func (c *components) ShowConfirmDialogWithDefault(title, message string, defaultValue bool) (bool, error) {
	if title != "" {
	...
	if err != nil {
		c.logger.Debugf("确认对话框交互失败: %v", err)
		return false, fmt.Errorf("确认对话框失败: %v", err)
	}

	c.logger.Debugf("确认对话框结果: %v", result)
...
func (c *components) ShowInputDialog(title, prompt string, isPassword bool) (string, error) {
	...
	// 显示标准化操作提示
	if isPassword {
		c.showStandardTip("password")
	} else {
		c.showStandardTip("input")
	}
	...
	if err != nil {
		// 改进错误处理
		c.logger.Debugf("输入对话框交互失败: %v, isPassword: %v", err, isPassword)
		if err.Error() == "interrupt" {
	...
	if isPassword {
		c.logger.Debugf("密码输入完成（长度: %d）", len(result))
	} else {
		c.logger.Debugf("文本输入完成: %s", result)
	}

	return result, nil
}

// ShowContinuePrompt 显示"按 Enter 键继续"的非确认提示
func (c *components) ShowContinuePrompt(title, message string) error {
	if title != "" {
		pterm.DefaultHeader.WithBackgroundStyle(pterm.NewStyle(c.theme.InfoColor)).
			Println(title)
		pterm.Println()
...
// showStandardTip 显示标准化的操作提示（内部辅助方法）
func (c *components) showStandardTip(tipType string) {
...
// Clear 实现 Components 接口 - 清屏
func (c *components) Clear() error {
	// 使用ANSI转义码清屏
	// \033[2J - 清除整个屏幕
	// \033[H  - 将光标移到左上角
