...
# 确保ONNX Runtime库文件存在
echo -e "${YELLOW}📦 检查ONNX Runtime依赖...${NC}"
bash "${ROOT_DIR}/scripts/build/ensure_onnx_libs.sh" || {
    echo -e "${RED}❌ ONNX Runtime库文件准备失败${NC}"
    exit 1
}
...
echo "1) 单节点开发模式 (端口: 28680)"
echo "2) 双节点集群模式 (端口: 28680, 8082)"
echo "3) 退出"
echo
...
case $choice in
  1)
    ...
    echo "HTTP API: http://localhost:28680"
    echo "WebSocket: ws://localhost:28681"
    echo "gRPC: localhost:28682"
    echo
  ...
  2)
    ...
    echo "节点1配置: $CFG1"
    echo "节点2配置: $CFG2"
    echo "节点1 API: http://localhost:28680"
    ...
    echo "节点1 PID: $NODE1_PID (http://localhost:28680)"
