...
⚠️ **架构兼容性说明**：如果您的Mac是Intel芯片（x86_64），但protoc是ARM64版本（通过Homebrew在ARM64终端中安装），会出现"Bad CPU type in executable"错误。

**解决方案**：
- **方案1（推荐）**：在x86_64终端中安装x86_64版本的protoc：
  ```bash
  arch -x86_64 /bin/bash -c "brew install protobuf"
  ```
- **方案2**：如果您的Mac支持ARM64，切换到ARM64终端：
  ```bash
  arch -arm64 zsh
  ```
  然后重新执行脚本
