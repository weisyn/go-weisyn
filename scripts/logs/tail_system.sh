#!/bin/bash
# 查看系统日志脚本
# 用途：快速查看系统日志（P2P、共识、存储等），过滤掉业务日志
...
SYSTEM_LOG="$LOG_DIR/node-system.log"

# 检查文件是否存在
if [ ! -f "$SYSTEM_LOG" ]; then
    echo "❌ 系统日志文件不存在: $SYSTEM_LOG"
    echo ""
    echo "提示："
...
if command -v jq &> /dev/null; then
    tail -f "$SYSTEM_LOG" | jq -r '.timestamp + " [" + .level + "] " + .message + (if .module then " [module=" + .module + "]" else "" end)'
else
    tail -f "$SYSTEM_LOG"
fi
