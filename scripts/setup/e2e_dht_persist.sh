...
# Node1: enable diagnostics on 18080, clear gater restrictions for local testing, add genesis config
jq '.p2p.diagnostics_enabled=true
  | .p2p.diagnostics_port=18080
  ...
  | .p2p.listen_addresses = ["/ip4/127.0.0.1/tcp/4001", "/ip4/127.0.0.1/udp/4001/quic-v1"]
...
# Node2: enable diagnostics on 18081, clear all gater restrictions for local testing, add genesis config, use separate storage
jq '.p2p.diagnostics_enabled=true
  | .p2p.diagnostics_port=18081
  | .p2p.advertise_private_addrs=true
  | .p2p.features.enable_nat_port=false
  | .p2p.features.enable_auto_relay=false
  | .p2p.features.enable_autonat=false
  ...
  | .p2p.gater.allowed_prefixes = ["/ip4/127.", "/ip4/10.", "/ip4/192.168.", "/ip6/fd"]
  | .p2p.gater.blocked_prefixes = []
  | .p2p.listen_addresses = ["/ip4/127.0.0.1/tcp/4002", "/ip4/127.0.0.1/udp/4002/quic-v1"]
...
# wait diagnostics
for i in {1..30}; do
  if curl -sf "http://127.0.0.1:18080/debug/host" >/dev/null; then break; fi
  sleep 1
  if [[ $i -eq 30 ]]; then 
...
HOST_JSON=$(curl -sf "http://127.0.0.1:18080/debug/host")
...
TCP_ADDR="/ip4/127.0.0.1/tcp/4001"
...
sleep 2
if ! lsof -iTCP:4001 -sTCP:LISTEN >/dev/null 2>&1; then
  echo "node1 is not listening on tcp:4001"
  tail -30 "$LOG1" || true
  exit 1
fi
...
# Wait and assert node2 port 4002 is listening
for i in {1..10}; do
  if lsof -iTCP:4002 -sTCP:LISTEN >/dev/null 2>&1; then break; fi
  sleep 1
  if [[ $i -eq 10 ]]; then
    echo "node2 is not listening on tcp:4002"
    tail -30 "$LOG2" || true
    exit 1
  fi
done
...
for i in {1..30}; do
  if curl -sf "http://127.0.0.1:18081/debug/host" >/dev/null; then break; fi
  sleep 1
  if [[ $i -eq 30 ]]; then 
...
HOST2_JSON=$(curl -sf "http://127.0.0.1:18081/debug/host")
