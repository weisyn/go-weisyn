...
**场景一：医疗AI诊断纠纷**
一家医院使用AI辅助诊断，患者质疑诊断结果。医院需要证明AI使用了正确的模型和输入数据，但AI决策是黑盒，无法追溯。如果能在区块链上记录AI的推理过程，就能提供不可篡改的证据链。但传统区块链无法运行AI模型——因为所有节点必须得到相同结果，而AI推理是非确定性的。

**场景二：电商订单支付失败**
用户下单后支付成功，但库存扣减失败，订单被取消。用户要求退款，但支付系统和库存系统数据不一致，无法确定责任。如果整个流程能在区块链上原子性执行，就不会出现这种问题。但传统区块链只能处理单笔交易，无法支持跨系统的长事务。

**场景三：制造业AI排产系统崩溃**
一个工厂想用AI优化生产排程，并将决策过程记录在链上。但传统区块链要求50个节点重复执行——这意味着AI模型要运行50次，数据库被调用50次，API被请求50次...系统直接崩溃，生产计划无法执行。
