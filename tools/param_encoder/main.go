...
func main() {
	if len(os.Args) < 2 {
		...
		fmt.Println("用法:")
		fmt.Println("  go run param_encoder.go transfer <to_address> <amount>")
		fmt.Println("  go run param_encoder.go balance <address>")
		fmt.Println("  go run param_encoder.go approve <spender> <amount>")
		fmt.Println("")
		fmt.Println("示例:")
		fmt.Println("  go run param_encoder.go transfer CWb1owGnpUaB2JoQPhohpa81Cz9aiqikZG 1000")
		fmt.Println("  go run param_encoder.go balance CUQ3g6P5WmFN289pPn7AAhnQ3T2cZRv2BR")
		return
	}
