...
func main() {
	if len(os.Args) < 2 {
		...
		fmt.Println("用法:")
		fmt.Println("  go run main.go generate <count>  - 生成指定数量的密钥对")
		fmt.Println("  go run main.go genesis          - 生成创世块密钥文件")
		fmt.Println("")
		fmt.Println("示例:")
		fmt.Println("  go run main.go generate 5")
		fmt.Println("  go run main.go genesis")
		return
	}
